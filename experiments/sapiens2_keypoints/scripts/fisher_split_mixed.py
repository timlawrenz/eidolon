...
d=np.load('/tmp/eidolon_3d_spike/study_features_mixed.npz',allow_pickle=True)
...
json.dump({'sapiens2_meanJ':float(Js.mean()),'n_morph':int(len(morph)),'n_trans':int(len(trans)),
           ...
           'dwpose_meanJ':float(Jd.mean())}, open('/tmp/eidolon_3d_spike/study_split_mixed.json','w'),indent=2)
print("\nSaved study_split_mixed.json")
