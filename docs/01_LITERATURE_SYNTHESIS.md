...
- **[[00 - articles/Latent-Identity Tuning in Text-to-Image Personalization Models|Latent-Identity Tuning in Text-to-Image Personalization Models]]** *Garibi et al. (TAU/Cornell)* (2026) — **Closest to Eidolon's Poser concept.** Explores Q-Former identity token spaces in IP-Adapter encoders (Omni-ID/PuLID + Flux.dev). Finds different tokens attend to different facial regions (eyes, nose, mouth). Uses PCA and SVM on token subspaces to discover editing directions for fine-grained identity tuning (beard, freckles, nose shape, eye openness). Edits persist across diverse prompts. **Relevance:** validates that structured token spaces carry localized semantics (→ Eidolon's expanded-token design §7.3); validates IP-Adapter + Flux.dev as a viable generation backbone; provides evaluation methodology template (ArcFace/AuraFace cosine similarity for identity consistency across prompts). **Does NOT compete:** edits existing identities (not novel generation), no mathematical orthogonality guarantees, no identity/geometry separation, no pose-invariance. [Full review](docs/reviews/paper_identity_tuning_garibi_2026.md). No public code yet — monitor `github.com/garibida/IdentityTuning`.
...
6. **IdentityTuning (Garibi et al., 2026)** is the closest work to Eidolon's Poser concept — token-space identity editing with PCA/SVM directions — and independently validates that (a) structured Q-Former token spaces carry localized facial semantics, (b) IP-Adapter + Flux.dev produces identity-consistent generation, and (c) PCA is a useful discovery tool in face latent spaces. However, it edits existing identities rather than generating novel ones, and offers no mathematical orthogonality, pose-invariance, or identity/geometry separation guarantees.
